...
class AWSConfig(TerraformStack):
    ...
    def enable_awsconfig_in_account(self, account_id, bucket_name):
        ...
        """
        # This ensures that you don’t get redundant
        # copies of IAM configuration Items in every Region.
        record_global_resources_region = (
            "us-gov-west-1" if "gov" in self.partition else "us-east-1"
        )
        iam_role_arn = self.__create_iam_role()
        ...
        print(f"AWS Config is enabled in all regions for account_id {account_id}")
    ...
    def get_available_regions(self):
        ...
        """
        sess = Session()
        aws_config_regions = sess.get_available_regions(
            "config", partition_name=self.partition, allow_non_regional=False
        )
...
def main():
    ...
    for account, profile in accounts_list.items():
        ...
        aws_config = AWSConfig(
            app, f"aws-config-{account}", profile_name=profile, partition=partition_name
        )
